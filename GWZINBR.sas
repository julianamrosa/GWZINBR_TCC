...
%macro Golden(DATA=,YVAR=,XVAR=,XVARINF=,WEIGHT=,LAT=,LONG=,OUTPUT=,GLOBALMIN=YES,METHOD=,MODEL=ZINB,BANDWIDTH=CV,OFFSET=,FORCE=YES,MAXG=100,DISTANCEKM=NO);
proc iml;
use &DATA;
...
if cont>1 then parg=1/(sum((y-uj)##2/uj)/(n-nvar));
...
do while (abs(ddev)>0.000001 & cont2<100);
	...
	if det(x`*(Ai#x))=0 then bg=j(ncol(x),1,0);
...
%if &XVARINF ne %then %do;
lambda0=(ncol(pos0)-sum((parg/(uj+parg))##parg))/n;
if lambda0<=0 then lambdag=j(ncol(G),1,0);
else do;
...
if nrow(pos0)=0 | any(lambdag)=0 then do;
if nrow(pos0)=0 then do;
pos0=pos1;
%if %upcase(&MODEL)=ZINB or %upcase(&MODEL)=ZIP %then %do;
call symputx("MODEL", "NEGBIN");       
%end;
end;
%if %upcase(&FORCE) ne YES %then %do;call symputx("MODEL", "NEGBIN");%end;
end;
%end;
...
%IF %UPCASE(&MODEL)=ZINB or %UPCASE(&MODEL)=ZIP %THEN %DO;
npar=sum(S)+sum(Si);
%if %UPCASE(&BANDWIDTH)=AIC %THEN %DO;
...
%macro GWZINBR(DATA=,YVAR=,XVAR=,XVARINF=,WEIGHT=,LAT=,LONG=,GRID=,DHV=,METHOD=,MODEL=ZINB,OFFSET=,DISTANCEKM=NO,FORCE=YES,INT_INF=YES,MAXG=100,H=);
proc iml;
use &DATA;
read all var {&YVAR} into y;
read all var {&XVAR} into x;
...
Iy=choose(y>0,1,y);Iy=1-Iy;Iy2=Iy;
pos0=loc(y=0);
pos02=loc(y=0);
pos1=loc(y>0);
...
if lambda0<=0 then do;
lambdag=j(ncol(G),1,0);
print "NOTE: Expected number of zeros ("(char(sum((parg/(uj+parg))##parg),6,2))") >= number of zeros ("(char(ncol(pos0),4,0))"). No Need of Zero Model.";
end;
else do;
print "NOTE: Expected number of zeros ("(char(sum((parg/(uj+parg))##parg),6,2))") < number of zeros ("(char(ncol(pos0),4,0))"). Zero Model Used.";
lambda0=log(lambda0/(1-lambda0));
lambdag=lambda0//j(ncol(G)-1,1,0);
end;
pargg=parg;
...
if any(lambdag)=0 then Iy=j(nrow(y),1,0);
...
if all(lambdag)>0 & alphag=1E-6 then II=II[2:nrow(II),2:nrow(II)];
else if any(lambdag)=0 & alphag>1E-6 then II=II[1:ncol(x)+1,1:ncol(x)+1];
else if any(lambdag)=0 & alphag=1E-6 then II=II[2:ncol(x)+1,2:ncol(x)+1];
...
/**** COMPUTING VARIANCE OF BETA AND LAMBDA ******/
	if det(x`*(w#Ai#x#wt))=0 then C=j(ncol(x),nrow(x),0);
	else
	C=inv(x`*(w#Ai#x#wt))*x`#(w#Ai#wt)`;
	Ci=j(ncol(G),1,0);
	%if %upcase(&MODEL)=ZIP or %upcase(&MODEL)=ZINB %then %do;
	if det(G`*(w#Aii#G#wt))=0 then Ci=j(ncol(G),nrow(G),0);
	else
	Ci=inv(G`*(w#Aii#G#wt))*G`#(w#Aii#wt)`;
	if any(lambda)=0 then Ci=j(ncol(G),n,0);
	%END;
	...
	if any(lambda)=0 then Iy=j(nrow(y),1,0);
	exphx=1+exp(njl);
	...
	if any(b)=0 & any(lambda)=0 then do;
	II=j(ncol(x)+ncol(G)+1,ncol(x)+ncol(G)+1,0);
	end;
	else if any(lambda)=0 then do;
	dbb=w#wt#(Iy#(-(par*g1x##par#g2x/hgx)##2+par**2*g1x##par#g2x##2#(1-1/uj)/hgx)-(1-Iy)#(par*g2x#(1+(y-uj)/(par+uj))));
	if det((X#dbb#dbb/Ai)`*X)=0 then II=-(I1#daa)`*I1||-(I1#dab)`*X||-(I1#dal)`*G//(-X`*(dab#I1)||-((X#dbb)`*X)||-(X#dlb)`*G)//(-G`*(dal#I1)||-G`*(X#dlb)||-(G#dll)`*G);
	else II=-(I1#daa)`*I1||-(I1#dab)`*X||-(I1#dal)`*G//(-X`*(dab#I1)||-((X#dbb)`*X)*inv((X#dbb#dbb/Ai)`*X)*((X#dbb)`*X)||-(X#dlb)`*G)//(-G`*(dal#I1)||-G`*(X#dlb)||-(G#dll)`*G);
	end;
	else
	II=-(I1#daa)`*I1||-(I1#dab)`*X||-(I1#dal)`*G//(-X`*(dab#I1)||-((X#dbb)`*X)||-(X#dlb)`*G)//(-G`*(dal#I1)||-G`*(X#dlb)||-(G#dll)`*G);
	if all(lambda)>0 & alpha=1E-6 then II=II[2:nrow(II),2:nrow(II)];
	else if any(lambda)=0 & alpha>1E-6 then II=II[1:ncol(x)+1,1:ncol(x)+1];
	else if any(lambda)=0 & alpha=1E-6 then II=II[2:ncol(x)+1,2:ncol(x)+1];
	
	if det(II)=0 then do;
	if all(lambda)>0 & alpha=1E-6 then do;
	II=II[1:ncol(x),1:ncol(x)];
	if det(II)=0 then varabetalambda=j(nrow(II),1,0)//j(ncol(G),1,0);
	else varabetalambda=vecdiag(inv(II))//j(ncol(G),1,0);
	end;
	else if any(lambda)=0 & alpha=1E-6 then do;
	II=II[1:ncol(x),1:ncol(x)];
	if det(II)=0 then varabetalambda=j(nrow(II),1,0)//j(ncol(G),1,0);
	else varabetalambda=vecdiag(inv(II))//j(ncol(G),1,0);
	end;
	...
	else varabetalambda=vecdiag(inv(II));
	...
	else if all(lambda)>0 & alpha=1E-6 then do;
...
%end;
	...
	m1=(i-1)*ncol(x)+1;
	m2=m1+(ncol(x)-1);
	if det(CCC[,1:ncol(x)]`*(CCC[,ncol(CCC)-1]#CCC[,1:ncol(x)]#CCC[,ncol(CCC)]))=0 then BB[m1:m2,]=j(ncol(x),nrow(x),0);
	else BB[m1:m2,]=inv(CCC[,1:ncol(x)]`*(CCC[,ncol(CCC)-1]#CCC[,1:ncol(x)]#CCC[,ncol(CCC)]))*CCC[,1:ncol(x)]`#(CCC[,ncol(CCC)-1]#CCC[,ncol(CCC)])`;
	%if %upcase(&MODEL)=ZIP or %upcase(&MODEL)=ZINB %then %do;
	CCCl=G||w||wt;
	m1=(i-1)*ncol(G)+1;
	m2=m1+(ncol(G)-1);
	if det(CCCl[,1:ncol(G)]`*(CCCl[,ncol(CCCl)-1]#CCCl[,1:ncol(G)]#CCCl[,ncol(CCCl)]))=0 then BBl[m1:m2,]=j(ncol(G),nrow(G),0);
	else
	BBl[m1:m2,]=inv(CCCl[,1:ncol(G)]`*(CCCl[,ncol(CCCl)-1]#CCCl[,1:ncol(G)]#CCCl[,ncol(CCCl)]))*CCCl[,1:ncol(G)]`#(CCCl[,ncol(CCCl)-1]#CCCl[,ncol(CCCl)])`;
	%end;
	%END;
	/*************************************/
	_w_=w;
...
print sigma2[label='Sigma2e'] root_mse[label='Root MSE'] v1[label='#GWR parameters'] nparmodel[label='#GWR parameters (model)'] v2[label='#GWR parameters (variance)'];
influence=S;
...
if stdli[j]=0 then tstati[j]=0;
else tstati[j] = li[j,2]/stdli[j];
end;
...
%END;
print dev[label='Deviance'] ll[label='Full Log Likelihood'] pctll adjpctll AIC AICc;
%END;
%IF %UPCASE(&MODEL)=POISSON or %UPCASE(&MODEL)=NEGBIN %THEN %DO;
if ncol(pos02)=0 then do;
pos0=pos1;
...
_beta2_=_beta_;
%IF %UPCASE(&MODEL)=NEGBIN or %UPCASE(&MODEL)=ZINB %THEN %DO;
...
rowname={"P25", "P50", "P75", "IQR"} colname={'Intercept' &xvar %IF %UPCASE(&MODEL)=NEGBIN or %UPCASE(&MODEL)=ZINB %THEN %DO;'alpha' %END;}],,
descriptb[label="Descriptive Statistics" rowname={"Mean", "Min", "Max"} 
colname={'Intercept' &xvar %IF %UPCASE(&MODEL)=NEGBIN or %UPCASE(&MODEL)=ZINB %THEN %DO;'alpha' %END;}];
...
print _malpha_[label="alpha-level=0.05"] _t_critical_[format=comma6.2 label="t-Critical"] df;
...
colname={&XVARINF}],,
%END;
descriptls[label="Descriptive Statistics of Zero Inflation Standard Errors" rowname={"Mean", "Min", "Max"} 
%IF %UPCASE(&INT_INF)=YES %THEN %DO;
colname={'Intercept' &XVARINF}];
%END;
%ELSE %DO;
...
%IF %UPCASE(&INT_INF)=YES %THEN %DO;
Vkl[label='' rowname={'Intercept' &XVARINF}
%END;
%ELSE %DO;
Vkl[label='' rowname={&XVARINF}
%END;
...
%IF %UPCASE(&MODEL)=ZINB %THEN %DO;
b2=bg//alphag;
if alphag=1E-6 then stdg=stdabetalambdag[1:ncol(x)]//(sqrt(1/abs(hessg))/(parg**2));
else stdg=stdabetalambdag[2:ncol(x)+1]//stdabetalambdag[1];
tg=b2/stdg;
dfg=nrow(y)-ncol(x);
probtg=2#(1-probt(abs(tg),dfg));

lambdag=lambdag;
if alphag=1E-6 then stdlambdag=stdabetalambdag[ncol(x)+1:nrow(stdabetalambdag)];
else stdlambdag=stdabetalambdag[ncol(x)+2:nrow(stdabetalambdag)];
...
%IF %UPCASE(&MODEL)=NEGBIN %THEN %DO;
b2=bg//alphag;
if alphag=1E-6 then stdg=stdabetalambdag[1:nrow(stdabetalambdag)]//(sqrt(1/abs(hessg))/(parg**2));
else stdg=stdabetalambdag[2:nrow(stdabetalambdag)]//stdabetalambdag[1];
tg=b2/stdg;
dfg=nrow(y)-ncol(x);
probtg=2#(1-probt(abs(tg),dfg));
p=ncol(x)+1;
...
bg_stdg=b2||stdg;
print "Global Parameter Estimates",,bg_stdg[label=' ' rowname={'Intercept' &xvar %IF %UPCASE(&MODEL)=NEGBIN or %UPCASE(&MODEL)=ZINB %THEN %DO;'alpha' %END;}
...
%END;
%end;
print varnamezip1[label="Parameter"] lambdag[label="Estimate" format=12.6]
	stdlambdag[label="Standard Error" format=12.6] tlambdag[label="t Value" format=12.2] 
	probtlambdag[label="Pr > |t|" format=pvalue6.4],,
"NOTE: The denominator degrees of freedom for the t tests is" dflg[label=' ']".";
%END;

%IF %UPCASE(&MODEL)=ZINB or %UPCASE(&MODEL)=ZIP %THEN %DO;
ll=sum(-log(1+exp(G[pos0,]*lambdag))+log(exp(G[pos0,]*lambdag)+(parg/(parg+exp(X[pos0,]*bg+offset[pos0,])))##parg))+
sum(-log(1+exp(G[pos1,]*lambdag))+lgamma(parg+y[pos1,])-lgamma(y[pos1,]+1)-lgamma(parg)+
y[pos1]#log(exp(X[pos1,]*bg+offset[pos1,])/(parg+exp(X[pos1,]*bg+offset[pos1,])))+parg*log(parg/(parg+exp(X[pos1,]*bg+offset[pos1,]))));
AIC=2*p-2*ll;
AICc=AIC+2*(p*(p+1)/(n-p-1));
llnull1=sum(-log(1+zkg[pos0,])+log(zkg[pos0,]+(parg/(parg+y[pos0,]))##parg))+
sum(-log(1+zkg[pos1,])+lgamma(parg+y[pos1,])-lgamma(y[pos1,]+1)-lgamma(parg)+
...
print devg[label='Deviance'] ll[label='Full Log Likelihood'] pctll adjpctll AIC AICc;
%END;
%IF %UPCASE(&MODEL)=POISSON or %UPCASE(&MODEL)=NEGBIN %THEN %DO;
...
ll=sum(-log(0+exp(G[pos0,]*lambdag))+log(0*exp(G[pos0,]*lambdag)+pos0x))+
sum(-log(0+exp(G[pos1,]*lambdag))+lgamma(parg+y[pos1,])-lgamma(y[pos1,]+1)-lgamma(parg)+
y[pos1]#log(exp(X[pos1,]*bg+offset[pos1,])/(parg+exp(X[pos1,]*bg+offset[pos1,])))+parg*log(parg/(parg+exp(X[pos1,]*bg+offset[pos1,]))));
...
create _parameters_ from bistdt[colname={"id" "B" "x" "y" "stdbi" "tstat" "probt"}];
append from bistdt;
_tstat_=_beta_;
do j=1 to nrow(_stdbeta_);
do k=1 to ncol(_stdbeta_);
if _stdbeta_[j,k]=0 then _tstat_[j,k]=0;
else _tstat_[j,k] = _beta_[j,k]/_stdbeta_[j,k];
end;
end;
...
do i=1 to n;
do j=1 to ncol(x);
if _probt_[i,j]<0.01*(nvar/v1) then _sig_[i,j]="significant at 99%";
else if _probt_[i,j]<0.05*(nvar/v1) then _sig_[i,j]="significant at 95%";
else if _probt_[i,j]<0.1*(nvar/v1) then _sig_[i,j]="significant at 90%";
else _sig_[i,j]="not significant at 90%";
if _probt_[i,j]=. then _sig_[i,j]="not significant at 90%";
end;
end;
...
if _stdlambda_[j,k]=0 then _tstatl_[j,k]=0;
else _tstatl_[j,k] = _lambda_[j,k]/_stdlambda_[j,k];
end;
end;
...
do i=1 to n;
do j=1 to ncol(G);
if _probtl_[i,j]<0.01*((nvar+ncol(G))/v1) then _sigl_[i,j]="significant at 99%";
else if _probtl_[i,j]<0.05*((nvar+ncol(G))/v1) then _sigl_[i,j]="significant at 95%";
else if _probtl_[i,j]<0.1*((nvar+ncol(G))/v1) then _sigl_[i,j]="significant at 90%";
else _sigl_[i,j]="not significant at 90%";
if _probtl_[i,j]=. then _sigl_[i,j]="not significant at 90%";
end;
end;
...
_label2_=repeat("Inf_std_",ncol(G))//repeat("Inf_tstat_",ncol(G))//repeat("Inf_probt_",ncol(G));
_colname_={"x" "y"}||_colname1_||concat(_label_,repeat(_colname1_`,3))`||_colname3_`||concat(_label2_,repeat(_colname2_`,3))`;
call change(_colname_, "_ ", "_");
call change(_colname_, "_ ", "_");
...
if alphai[j,3]=0 then atstat[j]=0;
else atstat[j] = alphai[j,2]/alphai[j,3];
end;
atstat=choose(atstat=.,0,atstat);
aprobtstat=2*(1-probnorm(abs(atstat)));
_siga_=j(n,1,"not significant at 90%");
do i=1 to n;
if aprobtstat[i]<0.01*(nvar/v1) then _siga_[i]="significant at 99%";
else if aprobtstat[i]<0.05*(nvar/v1) then _siga_[i]="significant at 95%";
else if aprobtstat[i]<0.1*(nvar/v1) then _siga_[i]="significant at 90%";
...
create _alpha_ from alphai[colname={"x" "y" "id" "alpha" "std" "tstat" "probt"}];
append from alphai;
%END;
_beta_=shape(bi[,2],m);
...
%if %upcase(&MODEL)=ZIP or %upcase(&MODEL)=ZINB %then %do;
data _parameters2_;merge _parameters2_ _sig_inf_parameters2_;run;
%end;
%IF %UPCASE(&MODEL)=NEGBIN or %UPCASE(&MODEL)=ZINB %THEN %DO;
...
%macro map2d(table=,var=,map=,id=,idtype=N,sampledata=,method=EQUAL,where=);
...
if &var<=&minp+(&maxp-&minp)/&np then color="&color1";
%do i=2 %to &np;
else if &var<=&minp+&i*(&maxp-&minp)/&np then color="&&color&i";
%end;
run;
%mend cl;
%cl;
...
%macro cl;
data anno&table;set anno&table;
if &var<=&minp+(&meanp-3*&stdp) then color="&color1";
else if &var<=&minp+&meanp-2*&stdp then color="&color2";
else if &var<=&minp+&meanp-&stdp then color="&color3";
else if &var<=&minp+&meanp+&stdp then color="&color4";
else if &var<=&minp+&meanp+2*&stdp then color="&color5";
else color="&color6";
run;
%mend cl;
%cl;
...
%macro cl;
data anno&table;set anno&table;
if &var<=&minn+(&maxn-&minn)/&nn then color="&color1";
%do i=2 %to &nn;
else if &var<=&minn+&i*(&maxn-&minn)/&nn then color="&&color&i";
%end;
run;
%mend cl;
%cl;
...
data anno&table;set _a_ anno&table;run;
data a;
%if %upcase(&idtype)=N %then %do;&id=0;%end;
%else %do;&id='0';%end;
v=2;
run;
